...
import { EXAM_CONFIG } from "@/lib/cambridge-b2-first/config";

describe("Cambridge B2 First Config", () => {
  it("has correct exam ID", () => {
    expect(EXAM_CONFIG.examId).toBe("EXAM018");
  });

  it("has correct slug", () => {
    expect(EXAM_CONFIG.slug).toBe("cambridge-b2-first");
  });
