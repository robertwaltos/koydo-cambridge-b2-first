...
import { emitPrismSignal } from "@/lib/cambridge-b2-first/cortex-signals";

describe("PRISM Signals", () => {
  it("dispatches custom event on window", () => {
    const spy = vi.fn();
    window.addEventListener("koydo:prism-signal", spy);
    emitPrismSignal({ eventType: "question_answered", activityType: "cambridge-b2-first" });
  ...
  it("includes correct event detail", () => {
    ...
    emitPrismSignal({ eventType: "exam_started", value: 1, activityType: "cambridge-b2-first" });
    expect(detail).toEqual({ eventType: "exam_started", value: 1, activityType: "cambridge-b2-first" });
